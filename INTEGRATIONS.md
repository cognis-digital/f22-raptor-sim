...
**f22-raptor-sim** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `f22-raptor-sim` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
f22-raptor-sim ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
f22-raptor-sim ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
f22-raptor-sim ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `f22-raptor-sim` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="f22-raptor-sim") for rec in records]   # records = your JSON output
